...
def main(config):
    ...
    for i in range(len(config.dataset_names)):
        ...
        # 'results/results_transformer/{config.output_names[i]}.json'
        ...
        for res in results:
            ...
            plt.plot(ema, label=f'$r$ = {r}')
            
        plt.legend()
        plt.title(config.dataset_names[i])
        plt.xlabel('Sample size, $k$')
        plt.ylabel(r"$\left| \mathcal{L}_{k+1}(\hat{\boldsymbol{w}}) - \mathcal{L}_k(\hat{\boldsymbol{w}}) \right|$")
        plt.yscale('log')
        plt.tight_layout()
        plt.savefig(f'figs/figs_transformer/{config.output_names[i]}_num_layers.pdf', bbox_inches='tight')
        ...
        plt.yscale('log')
        plt.tight_layout()
        plt.savefig(f'figs/figs_transformer/{config.output_names[i]}_hidden_size.pdf', bbox_inches='tight')
        """
